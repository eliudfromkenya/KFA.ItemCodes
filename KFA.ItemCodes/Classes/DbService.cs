    ...
    internal static class DbService
    {
        ...
        public static async Task SaveItem(string itemCode,string itemName, string supplier, bool isUpdate)
        {
            try
            {
                ...
                if (isUpdate)
                    cmd.CommandText = $"UPDATE `tbl_stock_items` SET `item_name`= '{itemName.Replace("'", "''")}',`distributor` = {(string.IsNullOrWhiteSpace(supplier) ? "NULL" : $"'{supplier.Replace("'", "''")}'")} WHERE `item_code` = '{itemCode}'";
                else
                    cmd.CommandText = $"INSERT INTO `tbl_stock_items`(`date_added`, `date_updated`, `originator_id`, `is_currently_enabled`, `item_code`, `distributor`, `group_id`, `item_name`, `is_active`) VALUES ({DateTime.Now.ToString("yyyyMMdd")}100756977, {DateTime.Now.ToString("yyyyMMdd")}100756977, 30000000123, 1, '{itemCode}', {(string.IsNullOrWhiteSpace(supplier) ? "NULL" : $"'{supplier.Replace("'", "''")}'")},'{itemCode[..2]}', '{itemName.Replace("'", "''")}', 1);";
                cmd.ExecuteNonQuery();
